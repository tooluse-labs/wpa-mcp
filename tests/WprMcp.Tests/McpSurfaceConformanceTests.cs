...
public sealed class McpSurfaceConformanceTests
{
    ...
    public void WindowPrimitiveAllowlist_CoversLegacyFormulas()
    {
        ...
        var allowlistPath = Path.Combine(
            repoRoot, "tests", "WprMcp.Tests", "Architecture", "window-primitive-allowlist.txt");
        ...
        Assert.Equal(
            new[] { "src/WprMcp/Core/TimeWindow.cs", "src/WprMcp/Core/TraceTime.cs" },
            entries.Where(entry => entry.Owner == "PERMANENT")
                .Select(entry => entry.Path)
                .OrderBy(path => path, StringComparer.Ordinal));

        var sourceRoot = Path.Combine(repoRoot, "src", "WprMcp");
    ...
    private static string LocateRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "WprMcp.sln")))
            {

...
public sealed class DependencyGovernanceTests
{
    ...
    [
        "src/WprMcp/WprMcp.csproj",
        "tests/WprMcp.Tests/WprMcp.Tests.csproj",
    ...
    public void GlobalJson_MatchesSelectedSdkAndDisablesRollForward()
    {
        ...
        var selectedSdk = selected["WprMcpSdkVersion"];
    ...
    public void EveryProject_UsesSelectedTargetFramework()
    {
        ...
        Assert.Equal("$(WprMcpTargetFramework)", Assert.Single(build.Descendants("TargetFramework")).Value);
        ...
        Assert.Equal("net10.0", selected["WprMcpTargetFramework"]);
        ...
        foreach (var projectPath in ExpectedProjects)
        {
            ...
            Assert.Empty(project.Descendants("WprMcpTargetFramework"));
        }
    }
    ...
    public void EveryPackageVersion_IsExactAndCentral()
    {
        ...
        var expected = FixedPackageVersions
            .Select(package => (package.Id, Version: package.Version == "selected" ? "$(WprMcpMcpSdkVersion)" : package.Version))
            .ToArray();
        ...
        Assert.Matches(new Regex(@"^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?$", RegexOptions.CultureInvariant), selected["WprMcpMcpSdkVersion"]);
    ...
    public void Moq_IsPinnedTo42072NotWildcard()
    {
        ...
        var testProject = ReadXml("tests/WprMcp.Tests/WprMcp.Tests.csproj");
    ...
    {
        var selected = ReadSelectedPlatform();
        var targetFramework = selected["WprMcpTargetFramework"];
    ...
    {
        var selected = ReadSelectedPlatform();
        var server = ReadXml("src/WprMcp/WprMcp.csproj");
        var central = ReadCentralVersions();
        using var normal = ReadJson("src/WprMcp/packages.lock.json");
        using var rid = ReadJson("src/WprMcp/packages.win-x64.lock.json");

        Assert.Equal(selected["WprMcpTargetFramework"], Assert.Single(normal.RootElement.GetProperty("dependencies").EnumerateObject()).Name);
        var ridGraphs = rid.RootElement.GetProperty("dependencies").EnumerateObject().ToArray();
        Assert.Equal(
            [selected["WprMcpTargetFramework"], $"{selected["WprMcpTargetFramework"]}/win-x64"],
        ...
        Assert.NotEqual(
            Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(RepoPath("src/WprMcp/packages.lock.json")))),
            Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(RepoPath("src/WprMcp/packages.win-x64.lock.json")))));
    }
    ...
    public async Task NormalAndRidLockedRestore_DoNotChangeLockFiles()
    {
        var lockPaths = ExpectedProjects
            ...
            .Append("src/WprMcp/packages.win-x64.lock.json")
            .ToArray();
        ...
        var normal = await RunDotNetAsync(
            "restore", "WprMcp.sln", "--locked-mode", "-m:1", "-nr:false",
        ...
        var rid = await RunDotNetAsync(
            "restore", "src/WprMcp/WprMcp.csproj", "-r", "win-x64", "--locked-mode",
    ...
    public void SelectedSdkProbeProject_UsesSelectedTfmMcpPackageAndNormalLock()
    {
        ...
        Assert.Empty(project.Descendants("WprMcpTargetFramework"));
        Assert.Empty(project.Descendants("WprMcpMcpSdkVersion"));
        Assert.Empty(project.Descendants("WprMcpProtocolProfile"));
        ...
        Assert.Equal(selected["WprMcpTargetFramework"], graph.Name);
        var mcp = graph.Value.GetProperty("ModelContextProtocol");
        Assert.Equal("Direct", mcp.GetProperty("type").GetString());
        Assert.Equal(selected["WprMcpMcpSdkVersion"], mcp.GetProperty("resolved").GetString());
    }
    ...
    public void WorkflowSdk_EqualsGlobalJsonExactly()
    {
        ...
        Assert.Contains("uses: ./.github/actions/setup-wprmcp", ReadRepoText(".github/workflows/quality.yml"), StringComparison.Ordinal);
        Assert.Contains("uses: ./.github/actions/setup-wprmcp", ReadRepoText(".github/workflows/release.yml"), StringComparison.Ordinal);
    }
    ...
    public void EarlyQuality_RunsNormalAndWinX64LockedRestoreBuildAndNonPackageTests()
    {
        ...
        Assert.Contains("uses: ./.github/actions/setup-wprmcp", quality, StringComparison.Ordinal);
        Assert.Contains("dotnet restore WprMcp.sln --locked-mode", quality, StringComparison.Ordinal);
        Assert.Contains("dotnet restore src/WprMcp/WprMcp.csproj -r win-x64 --locked-mode -p:NuGetLockFilePath=packages.win-x64.lock.json", quality, StringComparison.Ordinal);
        Assert.Contains("dotnet build WprMcp.sln -c Release --no-restore -warnaserror", quality, StringComparison.Ordinal);
        Assert.Contains("dotnet test WprMcp.sln -c Release --no-build --filter \"Category!=Package\"", quality, StringComparison.Ordinal);
    }
    ...
    {
        var quality = ReadRepoText(".github/workflows/quality.yml");
        var solutionTests = Regex.Matches(quality, @"(?m)^\s*run:\s*(?<command>dotnet test WprMcp\.sln[^\r\n]*)")
    ...
    private static Dictionary<string, string> ReadCentralVersions()
    {
        ...
        return ReadXml("Directory.Packages.props").Descendants("PackageVersion")
            .ToDictionary(
                ...
                package => package.Attribute("Version")!.Value == "$(WprMcpMcpSdkVersion)"
                    ? selected["WprMcpMcpSdkVersion"]
    ...
    private static async Task<(int ExitCode, string Output)> RunDotNetAsync(params string[] arguments)
    {
        ...
        {
            FileName = Environment.GetEnvironmentVariable("WPRMCP_DOTNET_HOST")

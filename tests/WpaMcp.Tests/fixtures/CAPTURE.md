...
```powershell
cd tests\WpaMcp.Tests\fixtures
...
```powershell
powershell.exe -ExecutionPolicy Bypass -File D:\wpa-mcp\tests\WpaMcp.Tests\fixtures\Capture-SmallWaitBound.ps1
```
...
```powershell
dotnet run --project tools\etlshrink -- tests\WpaMcp.Tests\fixtures\small_wait_bound.etl tests\WpaMcp.Tests\fixtures\small_wait_bound.shrunk.etl 500
```
...
```powershell
dotnet run --project src\WpaMcp -- --probe-stacks tests\WpaMcp.Tests\fixtures\small_wait_bound.etl
```
...
```powershell
powershell.exe -ExecutionPolicy Bypass -File D:\wpa-mcp\tests\WpaMcp.Tests\fixtures\Capture-SmallMemory.ps1
```

For the #13 fixture refresh path, prefer the guarded wrapper:

```powershell
powershell.exe -ExecutionPolicy Bypass -File D:\wpa-mcp\tests\WpaMcp.Tests\fixtures\Refresh-SmallMemoryFixture.ps1
```
...
```powershell
dotnet run --project src\WpaMcp -- --find-marker small_memory.etl "Memory/ProcessMemInfo" count_by_event 10
dotnet run --project src\WpaMcp -- --find-marker small_memory.etl "Object/" count_by_event 10
dotnet run --project src\WpaMcp -- --find-marker small_memory.etl "Pool/" count_by_event 10
dotnet run --project src\WpaMcp -- --find-marker small_memory.etl "0268a8b6-74fd-4302-9dd0-6e8f1795c0cf)/Opcode(32)" count_by_event 10
```
...
```powershell
powershell.exe -ExecutionPolicy Bypass -File D:\wpa-mcp\tests\WpaMcp.Tests\fixtures\Capture-JitOnly.ps1
```

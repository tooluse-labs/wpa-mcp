...
function Test-AnalyzerPoolCandidate {
    ...
    $oldFixturePath = $env:WPAMCP_MEMORY_FIXTURE_PATH
    try {
        $env:WPAMCP_MEMORY_FIXTURE_PATH = $validationPath
        Write-Host "Validating analyzer pool rows for candidate..."
        dotnet test WpaMcp.sln -c Release --no-build --filter "FullyQualifiedName~MemoryResourceAnalysis_ReturnsProcessSnapshotsAndHandleDeltas" | Out-Host
        $exitCode = $LASTEXITCODE
        return $exitCode -eq 0
    }
    finally {
        if ($null -eq $oldFixturePath) { Remove-Item Env:\WPAMCP_MEMORY_FIXTURE_PATH -ErrorAction SilentlyContinue }
        else { $env:WPAMCP_MEMORY_FIXTURE_PATH = $oldFixturePath }
...
try {
    Invoke-CommandChecked "Building WpaMcp release tool..." {
    ...
    if (-not $SkipTests) {
        ...
        Invoke-CommandChecked "Running memory fixture tests..." {
            dotnet test WpaMcp.sln -c Release --no-build --filter "FullyQualifiedName~MemoryResourceAnalysisTests|FullyQualifiedName~InspectTraceTests"
        }
    }
}
finally {

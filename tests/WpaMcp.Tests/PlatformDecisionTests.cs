...
public sealed class PlatformDecisionTests
{
    ...
    public void CandidateRunner_ReleaseUnitTestsExcludeOuterPlatformDecisionTests()
    {
        ...
        Assert.Contains(
            "'--filter', 'FullyQualifiedName!~WpaMcp.Tests.PlatformDecisionTests'",
    ...
    public void CandidateResult_AcceptsRetainedPublishAndLaunchFailuresButRejectsFalsePassClaims()
    {
        ...
        try
        {
            ...
            var serverPath = stdioArtifacts.Select(property => property.Key).Single(path => path.EndsWith("WpaMcp.exe", StringComparison.Ordinal));
    ...
    public void SdkCandidateProbe_UsesSelectedRevisionProfileAndPublicSdkSeamsOnly()
    {
        ...
        Assert.Equal("$(WpaMcpTargetFramework)", ProjectProperty(build, "TargetFramework"));
        Assert.Null(ProjectPackageVersion(project, "ModelContextProtocol"));
        Assert.Equal("$(WpaMcpMcpSdkVersion)", CentralPackageVersion(packages, "ModelContextProtocol"));
    ...
    public void CandidateRunner_UsesRealProductionStdioHandshakeAndRetainedLaunchIdentity()
    {
        ...
        var probe = File.ReadAllText(LocateRepoFile("tests", "WpaMcp.Tests", "PlatformProductionStdioTests.cs"));
        ...
        Assert.Contains("WPAMCP_PLATFORM_SERVER_PATH", runner, StringComparison.Ordinal);
        Assert.Contains("WPAMCP_PLATFORM_EXPECTED_LAUNCH_SHA256", runner, StringComparison.Ordinal);
        Assert.Contains("WPAMCP_PLATFORM_EVIDENCE_PATH", runner, StringComparison.Ordinal);
    ...
    public void SelectedPlatform_ValuesExactlyMatchCandidateResult()
    {
        ...
        Assert.Equal(result.GetProperty("sdkVersion").GetString(), selected["WpaMcpSdkVersion"]);
        Assert.Equal(result.GetProperty("targetFramework").GetString(), selected["WpaMcpTargetFramework"]);
        Assert.Equal(result.GetProperty("mcpSdkVersion").GetString(), selected["WpaMcpMcpSdkVersion"]);
        Assert.Equal(result.GetProperty("protocolRevision").GetString(), selected["WpaMcpProtocolRevision"]);
        Assert.Equal(result.GetProperty("protocolProfile").GetString(), selected["WpaMcpProtocolProfile"]);
    ...
    [
        "WpaMcpSdkVersion",
        "WpaMcpTargetFramework",
        "WpaMcpMcpSdkVersion",
        "WpaMcpProtocolRevision",
        "WpaMcpProtocolProfile",
    ];
    ...
    private static bool SelectedValuesMatchCandidate(IReadOnlyDictionary<string, string> selected, JsonElement candidate) =>
        selected["WpaMcpSdkVersion"] == candidate.GetProperty("sdkVersion").GetString() &&
        selected["WpaMcpTargetFramework"] == candidate.GetProperty("targetFramework").GetString() &&
        selected["WpaMcpMcpSdkVersion"] == candidate.GetProperty("mcpSdkVersion").GetString() &&
        selected["WpaMcpProtocolRevision"] == candidate.GetProperty("protocolRevision").GetString() &&
        selected["WpaMcpProtocolProfile"] == candidate.GetProperty("protocolProfile").GetString();
    ...
    {
        var configured = Environment.GetEnvironmentVariable("WPAMCP_DOTNET_HOST")
    ...
    private static string WriteResultFixture(IEnumerable<string> hostModes)
    {
        ...
        var serverPath = Path.Combine(serverPublish, "WpaMcp.exe");
        File.WriteAllBytes(serverPath, [9, 8, 7, 6]);
        File.WriteAllText(Path.Combine(serverPublish, "WpaMcp.deps.json"), JsonSerializer.Serialize(new
        {
...
public sealed class PlatformNonSdkRuntimeProbeTests
{
    ...
    public async Task GoldenTraceEventReads_OpensEveryFixtureFromTemporaryCopy()
    {
        ...
        var evidencePath = RequiredEnvironment("WPAMCP_PLATFORM_GOLDEN_EVIDENCE_PATH");
    ...
    public void NativeLayout_LoadsExactProductionAmd64Dependencies()
    {
        ...
        var publishRoot = RequiredEnvironment("WPAMCP_PLATFORM_PUBLISH_ROOT");
        var evidencePath = RequiredEnvironment("WPAMCP_PLATFORM_NATIVE_EVIDENCE_PATH");
        var serverPath = Path.Combine(publishRoot, "WpaMcp.exe");
    ...
    public async Task WindowsDiaPdbResolution_EnumeratesFunctionAndResolvesItsRva()
    {
        if (!PlatformProbeRequired()) return;
        Assert.Equal(Architecture.X64, RuntimeInformation.ProcessArchitecture);
        var root = RequiredEnvironment("WPAMCP_PLATFORM_DIA_ROOT");
        var evidencePath = RequiredEnvironment("WPAMCP_PLATFORM_DIA_EVIDENCE_PATH");
        var msdiaPath = RequiredEnvironment("WPAMCP_PLATFORM_MSDIA_PATH");
        Directory.CreateDirectory(root);
    ...
    private static bool PlatformProbeRequired() =>
        string.Equals(Environment.GetEnvironmentVariable("WPAMCP_PLATFORM_REQUIRED"), "1", StringComparison.Ordinal);

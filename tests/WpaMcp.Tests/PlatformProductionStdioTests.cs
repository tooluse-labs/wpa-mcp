...
public sealed class PlatformProductionStdioTests
{
    ...
    {
        var serverPath = Environment.GetEnvironmentVariable("WPAMCP_PLATFORM_SERVER_PATH");
        if (string.IsNullOrWhiteSpace(serverPath))
        {
            if (Environment.GetEnvironmentVariable("WPAMCP_PLATFORM_REQUIRED") == "1")
            {
                throw new InvalidOperationException("WPAMCP_PLATFORM_SERVER_PATH is required in runner probe mode.");
            }
            return;
        }

        var publishRoot = RequireEnvironment("WPAMCP_PLATFORM_PUBLISH_ROOT");
        var protocolRevision = RequireEnvironment("WPAMCP_PLATFORM_PROTOCOL_REVISION");
        var protocolProfile = RequireEnvironment("WPAMCP_PLATFORM_PROTOCOL_PROFILE");
        var evidencePath = RequireEnvironment("WPAMCP_PLATFORM_EVIDENCE_PATH");
        var rawStdoutPath = RequireEnvironment("WPAMCP_PLATFORM_RAW_STDOUT_PATH");
        var rawStderrPath = RequireEnvironment("WPAMCP_PLATFORM_RAW_STDERR_PATH");
        var expectedLaunchSha256 = RequireEnvironment("WPAMCP_PLATFORM_EXPECTED_LAUNCH_SHA256");
        var requestedRuntimeIdentifier = RequireEnvironment("WPAMCP_PLATFORM_REQUESTED_RID");
        var publishRuntimeIdentifier = RequireEnvironment("WPAMCP_PLATFORM_PUBLISH_RID");

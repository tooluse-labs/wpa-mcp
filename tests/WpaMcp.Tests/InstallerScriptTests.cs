...
public class InstallerScriptTests
{
    ...
    [Fact]
    public void SetupScriptBootstrapsDotNet10ForTheNet10Server()
    {
        var content = File.ReadAllText(LocateScript("setup.ps1"));

        Assert.Contains("Microsoft.DotNet.SDK.10", content);
        Assert.Contains("Microsoft.DotNet.Runtime.10", content);
        Assert.Contains("Channel = '10.0'", content);
        Assert.Contains("Microsoft\\.NETCore\\.App 10\\.", content);
        Assert.DoesNotContain("Microsoft.DotNet.SDK.8", content);
        Assert.DoesNotContain("Microsoft.DotNet.Runtime.8", content);
    }

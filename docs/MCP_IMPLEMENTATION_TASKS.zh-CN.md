...
- **状态：** 进行中（2026-05-16）。`MemoryCapture.wprp` 和 `small_memory.etl` 已覆盖正向 `Memory/ProcessMemInfo` 与 handle-event 路径。`memory_resource_analysis` 现在会在存在 `PoolAllocation` / `PoolFree` events 时输出 observed paged/non-paged pool allocation/free deltas。当前 shrunk fixture 的 Pool 解析对转换环境敏感（CI 能看到 pool events，一个本地转换看不到），因此在提交更小的 no-stack 重抓 fixture 前，严格 pool-positive fixture 断言仍保持条件化。Pool rows 明确是 captured-window deltas，不是绝对 current counters，因为可用的 pool snapshot events 没有暴露可用 counter 字段。
...
- **v10 (2026-05-16)**：推进 T2.4：新增正向 `small_memory.etl` fixture 覆盖 `Memory/ProcessMemInfo` 和 handle events，并实现 observed pool allocation/free delta rows。当前 shrunk fixture 的 Pool event 可见性受转换环境影响，因此 pool-positive fixture 断言暂时保持条件化。

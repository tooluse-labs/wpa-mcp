...
- **状态：** ✅ 已完成（2026-05-16）。`MemoryCapture.wprp` 和 `small_memory.etl` 已覆盖正向 `Memory/ProcessMemInfo`、handle-event 与 observed pool allocation/free 路径。`memory_resource_analysis` 同时支持命名的 `Pool/...` events，以及 clean conversion 后保留下来的 raw classic Pool task GUID/opcode records，因此当前提交的 fixture 删除 `.etlx` 后也能证明 pool rows。Pool rows 明确是 captured-window deltas，不是绝对 current counters，因为可用的 pool snapshot events 没有暴露可用 counter 字段。

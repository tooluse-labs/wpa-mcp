...
`ActiveToolCatalog` joins those files to SDK-created typed tool methods and generated schemas at startup. The joined immutable catalog is the only production source for `tools/list`, calls, capability routing, schemas, annotations, pagination metadata, documentation projections, and snapshots. Assembly reflection may discover typed implementations during catalog construction, but no other component may independently infer a second tool list. Missing or duplicate tools, dangling capabilities, invalid schema pointers, evidence-reference drift, or annotation mismatch fail before the transport reads stdin.
...
The next structured result contract is literal `"2.0"`. Contract mode is selected once at server startup; individual calls cannot override it. The two transition modes are `legacy` and `2.0`.
...
`structuredContent` and the single text content block are two serializations of the same final redacted, paged, and fitted envelope and must be JSON-semantic equals on every success and failure. A summary, pointer, or structured-only fallback is forbidden because it creates two observable contracts. If the mirrored frame cannot fit, the server uses only a manifest-declared retrievable cursor boundary; otherwise it returns an atomic mirrored failure. An already-failed result retains its stable error code while budget-only evidence details are compacted. Legacy structured tools retain their Phase 0 output schemas and result shapes until the atomic contract-mode transition; legacy goldens are a compatibility floor, not a reason to preserve known incorrect facts.
...
`tools/list` has deterministic `discoveryPriority`, domain, and ordinal-name ordering. Bootstrap orientation tools are first, but later pages remain part of the complete catalog. Pages are fitted against the approved complete JSON-RPC frame limit with one tool plus its complete schemas as the indivisible unit. A single indivisible tool that cannot fit causes startup failure.

All pages combined contain every enabled tool exactly once. The server never changes the list for a loaded trace and never hides tools to meet prompt budgets. A client that does not follow `nextCursor` is recorded as incompatible. CI separately gates maximum page bytes and aggregate catalog bytes/tokens; successful paging does not claim lower aggregate prompt cost.
...
- Contract 2.0 and paged discovery are introduced behind startup mode while Phase 0 legacy goldens remain enforced.
- The default switches only after every active tool has a schema-valid structured result and the supported-client paging matrix passes.
- Contract 3.0 removes deprecated unsafe numeric projections and the legacy envelope after the release window fixed by ADR 0005.
- Any registry expansion or semantic strengthening is a versioned manifest/ADR change with snapshots and adversarial tests.

## Consequences

The implementation pays the cost of a larger, fully described tool surface and stateful cursor registries. In exchange, no tool, schema, omission, unsafe identifier, or inference boundary must be guessed by the model. The catalog and result adapter can now be implemented without unresolved public-wire choices.
